...
[.lead]
During Stage 5, you install and boot node4, check that the cluster and node-management ports from node2 come online on node4, verify the node4 installation, and restore key-manager configuration. You also relocate node2 NAS data LIFs and non-root aggregates from node3 to node4.
