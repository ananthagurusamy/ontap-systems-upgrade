...
--
Choose an upgrade procedure for a system with internal drives, which are listed below. If your system is not listed, see the https://hwu.netapp.com[NetApp Hardware Universe^] to check if it has internal drives. 

[cols=3*,options="header",20,20,20]
|===
...
--
Choose an upgrade procedure for a system with only external drives.

<Include a list of these systems>
...
*Note3*: When upgrading by moving volumes, you join and then unjoin the cluster. A switch is required for a two-node switchless cluster upgrade.
====
