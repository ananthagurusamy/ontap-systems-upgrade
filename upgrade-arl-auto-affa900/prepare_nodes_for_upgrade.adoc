...
. Verify that you have a supported service processor or baseboard management controller (BMC) firmware version for your upgrade by running the `service-processor show` command.
+
...
If you do not have new license keys or NetApp License File (NLF), currently licensed features in the cluster are available to the new controller. However, using unlicensed features on the controller might put you out of compliance with your license agreement, so you should install the new license key or NLF for the new controller after the upgrade is complete.
