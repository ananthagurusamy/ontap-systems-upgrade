...
.	[[A900_boot_node2]]If NetApp Storage Encryption (NSE) is in use on this configuration, the `setenv bootarg.storageencryption.support` command must be set to `true`, and the `kmip.init.maxwait` variable must be set to `off` to avoid a boot loop after the node2 configuration is loaded:
+
...
+
`boot_ontap menu`
.	Reassign the AFF A700 node2 disks to the AFF A900 node2 by entering "22/7" and selecting the hidden option `boot_after_controller_replacement` when the node stops at the boot menu.
+
...
[NOTE]
====
In the preceding console output example, ONTAP will prompt you for the partner node name if the system uses ADP disks.
