...
It is not possible to check the version of ONTAP installed on the boot media of the replacement controller module unless it is installed in a chassis and powered ON. The ONTAP version on the replacement system boot media must be same as the ONTAP version running on the old system that you are upgrading and both the primary and backup boot images must match. To verify the minimum supported ONTAP version for you upgrade, see link:guidelines_for_upgrading_controllers_with_arl.html[Supported upgrades for ARL].
