...
summary: After swapping the corresponding AFF A900 node1 controller module and NVRAM, you must netboot node1.
---
...
[.lead]
After swapping the corresponding AFF A900 node1 controller module and NVRAM, you must netboot node1. The term netboot means that you are booting from an ONTAP image stored on a remote server. When preparing for netboot, you must add a copy of the ONTAP 9 boot image onto a web server that the system can access.
