...
You can nondisruptively upgrade controller hardware on a high availability (HA) pair of AFF A700 nodes to an AFF A900 system by swapping the controller module and the NVRAM module. You migrate non-root aggregates between the AFF A700 nodes. After installation, you then migrate the non-root aggregates from the AFF A700 nodes to the AFF A900 nodes. The data hosted on the nodes that you are upgrading is accessible during the upgrade procedure.
