...
|Converts an AFF A700 to an AFF A900, keeping the A700 chassis
|Using `system controller replace` commands
|9.10.1 P3 or later
9.11.1 or later
|FAS9000 to FAS9500
|Converts an FAS9000 system to an FAS9500 system, keeping the FAS9000 chassis.
|===

You can nondisruptively upgrade controller hardware on a high availability (HA) pair by swapping the controller module and the NVRAM module for the following systems:

* AFF A700 nodes to an AFF A900 system
* FAS9000 nodes to an FAS9500 system

You migrate the non-root aggregates between the old controller nodes and the replacement controller nodes. After installation, you then migrate the non-root aggregates from the AFF A700 to the AFF A900 nodes or from the FAS9000 to the FAS9500 nodes. The data hosted on the nodes that you are upgrading is accessible during the upgrade procedure.

.About this task
During this controller upgrade procedure, you swap only the two field replaceable units (FRUs) on each node on the AFF A700 or FAS9000 system with the new FRUs. You do not need to move, disconnect, or reconnect the I/O cards, data cables, disk shelves, and disks.
