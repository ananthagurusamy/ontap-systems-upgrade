...
[.lead]
Node1 with the AFF A900 controller module and NVRAM module is now ready for boot up. Upgrading from an AFF A700 to an AFF A900 by swapping the controller module and NVRAM module involves moving only the console and management connections. This section provides the steps required to boot node1 with the AFF A900 controller module and NVRAM module.
...
+
NOTE: If the system reports `diskown.errorDuringIO` error messages, reset `setenv bootarg.storageencryption.support` in <<aff_a900_boot_node1,Step 1>> to `true` and run the `boot_ontap` command.
// BURT-1481586 30-May-2022
+
To reassign the AFF A700 node1 disks to AFF A900 node1, at the prompt, enter the actual node name of node1. Use the following example as a reference:
+
----
