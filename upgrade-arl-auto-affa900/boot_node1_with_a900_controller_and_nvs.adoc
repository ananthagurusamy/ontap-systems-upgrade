...
= Boot node1 with the AFF A900 or FAS9500 controller module and NVRAM module
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Node1 with the AFF A900 or FAS9500 controller module and NVRAM module is now ready for boot up. Upgrading from an AFF A700 or FAS9000 to an AFF A900 or FAS9500 by swapping the controller module and NVRAM module involves moving only the console and management connections. This section provides the steps required to boot node1 with the AFF A900 or FAS9500 controller module and NVRAM module.
...
+
`boot_ontap menu`
.	The node stops at the boot menu. Enter "22/7" and select the hidden option `boot_after_controller_replacement`. To reassign the node1 disks from the AFF A700 to AFF A900 or from the FAS9000 to FAS9500, at the prompt, enter the actual node name of node1. Use the following example as a reference:
+
----
