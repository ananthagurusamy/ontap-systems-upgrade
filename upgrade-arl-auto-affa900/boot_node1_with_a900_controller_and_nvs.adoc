...
+
`boot_ontap menu`
.	Reassign the AFF A700 node1 disks to the AFF A900 node1 by entering "22/7" and selecting the hidden option `boot_after_controller_replacement` when the node stops at the boot menu.
+
...
[NOTE]
====
In the preceding console output example, ONTAP will prompt you for the partner node name if the system uses Advanced Disk ADP disks.
