...
NOTE: During an AFF A700 to AFF A900 upgrade, if the system identifies the new controller as FAS9500 when you run the `system controller replace resume` command, you must set the `bootarg.init.flash_optimized` to `true` and save the new setting by using it the `saveenv` command at the Loader prompt.
