...
If you have a system with more than two cluster ports per node, such as an FAS8080 or an AFF8080 system, you must migrate and re-home the cluster LIFs to two cluster ports per node before you start the controller upgrade.If you perform the controller upgrade with more than two cluster ports per node, cluster LIFs might be missing on the new controller after the upgrade.
