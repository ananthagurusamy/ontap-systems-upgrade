...
summary: "Move node1's data LIFs to node2 if you have a two-node cluster, or to a third node if your cluster has more than two nodes, during a manual ARL upgrade on controllers running ONTAP 9.8 or later."
---
...
+
`network interface modify -vserver _Vserver-name_ -lif _LIF_name_ -home-node <node2> -home-port _port_name_`
