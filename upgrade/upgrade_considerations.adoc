...
NOTE: You can upgrade integrated systems by moving data to new storage (moving volumes) or converting the existing integrated system into a shelf then migrating it to a new system (moving storage). For example, you can upgrade a FAS26xx to a FAS8300. In either case, the data migration or converted shelf must remain in the same switched cluster.
