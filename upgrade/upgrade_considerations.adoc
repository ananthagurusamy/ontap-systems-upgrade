...
* The original nodes must support the same software version as the new nodes.
+
link:https://docs.netapp.com/us-en/ontap/upgrade/index.html[Upgrade ONTAP^]
+
NOTE: For details on mixed software version cluster upgrades, see https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html[Mixed version ONTAP clusters^].
