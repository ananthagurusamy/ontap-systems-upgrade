...
. [[assign_lifs]]If you are replacing an ONTAP node with a different model, you must reassign the management or intercluster LIFs configured on the network ports on the original system that will be used as high availability (HA) ports on the replacement system.
+
