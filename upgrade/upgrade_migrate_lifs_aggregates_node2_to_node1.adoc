...
[NOTE]
====
* You must netboot and install the ONTAP version on the AFF A400 that is identical to the version running on the AFF A250.
* Both the primary and backup boot image of the AFF A400 must have the same ONTAP version.
* If the AFF A400 cluster was previously configured, you must clear any residual cluster configuration by performing a `wipeconfig` from the boot menu.
====
