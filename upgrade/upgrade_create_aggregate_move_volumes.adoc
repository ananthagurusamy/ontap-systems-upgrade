...
[NOTE]
====
If your storage configuration includes SAN volumes, follow the steps in https://docs.netapp.com/us-en/ontap-metrocluster/transition/task_move_linux_iscsi_hosts_from_mcc_fc_to_mcc_ip_nodes.html[Move Linux iSCSI hosts from MetroCluster FC to MetroCluster IP nodes^] to move these volumes to the new nodes.
====
...
. If your storage configuration includes volumes with encryption enabled, follow the steps in https://docs.netapp.com/us-en/ontap/encryption-at-rest/encrypt-existing-volume-task.html[Enable encryption on an existing volume with the volume move start command^].
