...
If your storage configuration includes SAN volumes or encrypted volumes, you must also move these volumes:

* <<move_san_vols,Move SAN volumes>>
...
To move SAN volumes, follow the steps in https://docs.netapp.com/us-en/ontap-metrocluster/transition/task_move_linux_iscsi_hosts_from_mcc_fc_to_mcc_ip_nodes.html[Moving Linux iSCSI hosts from MetroCluster FC to MetroCluster IP nodes^]. This applies to moving SAN volumes in both MetroCluster and non-MetroCluster configurations. 

== Move volumes with encryption enabled

To move volumes with aggregate encryption enabled, follow the steps in https://docs.netapp.com/us-en/ontap/encryption-at-rest/encrypt-existing-volume-task.html[Enable encryption on an existing volume with the volume move start command^].
