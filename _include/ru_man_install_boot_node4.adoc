...
However, do not netboot node4 if it has the same or later version of Data ONTAP 9 that is installed on node2.
...
. If you are upgrading from a system with external disks to a system that supports internal and external disks (A800 systems, for example), set node4 as root to confirm it boots from the root aggregate of node2.
+
