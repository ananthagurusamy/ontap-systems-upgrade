...
[WARNING]
====
*As soon as the controller upgrade completes on the HA pair, you must unset the `kmip.init.maxwait` variable. See xref:ensure_new_controllers_are_set_up_correctly.adoc[Confirm that the new controllers are set up correctly]. Failure to do so might cause data loss if a power outage occurs.*
====
+
[NOTE]
====
