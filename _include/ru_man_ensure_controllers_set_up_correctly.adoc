...
. [[unset_maxwait_manual]]If self-encrypting drives are being used in the configuration and you have set the `kmip.init.maxwait` variable to `off` (for example, in link:install_boot_node3.html#step16[Step 16] of _Install and boot node3_), you must unset the variable:
+
