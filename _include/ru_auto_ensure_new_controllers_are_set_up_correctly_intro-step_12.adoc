...
. [[unset_maxwait_system_commands]]If self-encrypting drives are being used in the configuration and you have set the `kmip.init.maxwait` variable to `off` (for example, in link:install_boot_node4.html#step27[Install and boot node4, Step 27]), you must unset the variable:
+
