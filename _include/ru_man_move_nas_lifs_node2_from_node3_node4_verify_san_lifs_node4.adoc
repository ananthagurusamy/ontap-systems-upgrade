...
. [[worksheet_step2_node2]]If the cluster is configured for SAN LIFs, record the SAN LIFs and existing configuration information in the link:worksheet_information_before_moving_san_lifs.html[worksheet] for use later in the procedure.
...
+
NOTE: If a SAN LIF in the new configuration is not on a physical port that is attached to the same switch, it might cause a system outage when you reboot the node.
+
