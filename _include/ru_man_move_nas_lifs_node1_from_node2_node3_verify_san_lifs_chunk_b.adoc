...
.. Verify that the new port, switch, and adapter configurations are correct by comparing the output from the `fcp adapter show` command with the new configuration information recorded in the worksheet in <<worksheet_step2,Step 2>>. List the new SAN LIF configurations on node 3:
+
